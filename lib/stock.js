...
function listStocks(stockCodes) {
    return stockCodes.reduce(function(soFar, code) {
        return soFar.then(function() {
            var queryCode = resolveQueryCode(code);
            if(!queryCode) return Q();
...
function query(queryCode) {
    var d = Q.defer();
    var opt = {
        url : requestMarketUrl + queryCode,
        encoding : null
    };
    request(opt, function (error, response, body) {
        var statusCode = response.statusCode;
...
function parseResponseData(response) {
    var matchData = response.match(/.*_([a-z]{2})(\d+).*="(.*)";/);
    if(!matchData || !matchData[1]) {
        return Q();
    }
    var values = matchData[3].split('~');
    var stockInfo = {
        'name' : values[1],
        'price' : values[3],
        'upgrade' : values[32],
        ...
        'low' : values[34],
        'pe' : values[39],
        'value' : matchData[1] == 'hk' ? values[44] : values[45],
        'pb' : values[46]
    };
    stockInfo.code = matchData[1] + matchData[2];
    if(matchData[1] == 'hk') {
        ...
        stockInfo.market = (matchData[1] == 'sh' ? '上海' : '深圳');
    }
